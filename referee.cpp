...
struct Player {
    ...
    vector<Card> cards;      // Purchased cards
    vector<Card> reserved;   // Reserved cards (max 3)
    vector<Noble> nobles;    // Acquired nobles
...
struct GameState {
    ...
    vector<Card> deck_level1;         // Level 1 deck
    vector<Card> deck_level2;         // Level 2 deck
    vector<Card> deck_level3;         // Level 3 deck
    
    vector<Card> faceup_level1;       // 4 visible level 1 cards
    vector<Card> faceup_level2;       // 4 visible level 2 cards
    vector<Card> faceup_level3;       // 4 visible level 3 cards
    
    vector<Noble> available_nobles;   // 3 nobles in play
...
struct ValidationResult {
    ...
    ValidationResult(bool v = true, string msg = "") 
...
// Forward declarations for move validation (to be implemented later)
...
void checkAndAssignNobles(GameState& state, int player_idx, int noble_id = -1);

// Parse move from single-line string format
Move parseMove(const string& move_string, int player_id) {
    Move move;
    ...
    if (tokens.empty()) {
        cerr << "Error: Empty move string" << endl;
        return move;
    }
    ...
    if (action == "TAKE") {
        ...
        if (tokens.size() < 2) {
            cerr << "Error: RESERVE missing card_id" << endl;
            return move;
        }
        
        move.card_id = stoi(tokens[1]);
        ...
        if (tokens.size() < 2) {
            cerr << "Error: BUY missing card_id" << endl;
            return move;
        }
        ...
        if (noble_idx < tokens.size() && noble_idx + 1 < tokens.size()) {
            move.noble_id = stoi(tokens[noble_idx + 1]);
        }
        
    } else {
        cerr << "Error: Unknown move action: " << action << endl;
    }
    ...
    switch (move.type) {
        ...
        case BUY_CARD: {
            ...
            if (target_card) {
                ...
                // Check and assign nobles (pass noble_id from move)
                checkAndAssignNobles(state, player_idx, move.noble_id);
            }
            break;
        }
    }
...
void checkAndAssignNobles(GameState& state, int player_idx, int noble_id) {
    Player& player = state.players[player_idx];
    ...
    if (qualifying_noble_indices.empty()) {
        ...
        if (noble_id == -1) {
            ...
            cerr << "Multiple nobles qualify, randomly assigning noble " 
...
vector<Card> loadCards(const string& filename) {
    vector<Card> cards;
...
vector<Noble> loadNobles(const string& filename) {
    vector<Noble> nobles;
    ifstream file(filename);
    if (!file.is_open()) {
        cerr << "Error: Could not open " << filename << endl;
...
void initializeGame(GameState& state, unsigned int seed = 0) {
    ...
    cerr << "Initializing game with seed: " << seed << endl;
    
    // Load all cards
    vector<Card> all_cards = loadCards("cards.json");
    cerr << "Loaded " << all_cards.size() << " cards" << endl;
    ...
    cerr << "Level 1: " << level1.size() << " cards" << endl;
    cerr << "Level 2: " << level2.size() << " cards" << endl;
    cerr << "Level 3: " << level3.size() << " cards" << endl;
    ...
    for (int i = 0; i < 4 && i < level1.size(); i++) {
        state.faceup_level1.push_back(level1[i]);
    }
    for (int i = 4; i < level1.size(); i++) {
        state.deck_level1.push_back(level1[i]);
    }
    
    for (int i = 0; i < 4 && i < level2.size(); i++) {
        state.faceup_level2.push_back(level2[i]);
    }
    for (int i = 4; i < level2.size(); i++) {
        state.deck_level2.push_back(level2[i]);
    }
    
    for (int i = 0; i < 4 && i < level3.size(); i++) {
        state.faceup_level3.push_back(level3[i]);
    }
    for (int i = 4; i < level3.size(); i++) {
        state.deck_level3.push_back(level3[i]);
    }
    
    cerr << "Face-up cards drawn: " << state.faceup_level1.size() 
         << " (L1), " << state.faceup_level2.size() 
         << " (L2), " << state.faceup_level3.size() << " (L3)" << endl;
    
    // Load and shuffle nobles
    vector<Noble> all_nobles = loadNobles("nobles.json");
    cerr << "Loaded " << all_nobles.size() << " nobles" << endl;
    
    shuffle(all_nobles.begin(), all_nobles.end(), rng);
    
    // Draw 3 nobles
    for (int i = 0; i < 3 && i < all_nobles.size(); i++) {
        state.available_nobles.push_back(all_nobles[i]);
    }
    
    cerr << "Nobles in play: " << state.available_nobles.size() << endl;
    ...
    cerr << "Bank initialized: " << state.bank.total() << " total gems" << endl;
    
    // Initialize player gems to 0 (already default initialized)
    cerr << "Players initialized with 0 gems" << endl;
    
    state.current_player = 0;
    state.move_number = 0;
    
    cerr << "Game initialization complete!" << endl;
}

// Helper function to print game state (for testing)
void printGameState(const GameState& state) {
    cout << "\n=== GAME STATE ===" << endl;
    cout << "Move: " << state.move_number << ", Current Player: " << state.current_player << endl;
    ...
    for (const Card& card : state.faceup_level1) {
        cout << "  Card #" << card.id << ": " << card.color << " (" << card.points << " pts)" << endl;
    }
    ...
    cout << "\nNobles:" << endl;
    for (const Noble& noble : state.available_nobles) {
        cout << "  Noble #" << noble.id << " (" << noble.points << " pts)" << endl;
    }
    
    cout << "\nPlayer 0: " << state.players[0].points << " points, " 
         << state.players[0].tokens.total() << " gems" << endl;
    cout << "Player 1: " << state.players[1].points << " points, " 
...
// Print game state as JSON to stdout (for a specific viewer)
void printJsonGameState(const GameState& state, int viewer_id = 1) {
    cout << gameStateToJson(state, viewer_id) << endl;
}

// Process SETUP commands for replay mode
void processSetupCommands(GameState& state, vector<Card>& all_cards, vector<Noble>& all_nobles) {
    string line;
    ...
    while (getline(cin, line)) {
        ...
        if (command == "BEGIN") {
            ...
            if (!level1_setup || !level2_setup || !level3_setup || !nobles_setup) {
                cerr << "ERROR: Cannot BEGIN - incomplete setup!" << endl;
                cerr << "  level1 face-up cards: " << (level1_setup ? "✓" : "✗") << endl;
                cerr << "  level2 face-up cards: " << (level2_setup ? "✓" : "✗") << endl;
                cerr << "  level3 face-up cards: " << (level3_setup ? "✓" : "✗") << endl;
                cerr << "  nobles: " << (nobles_setup ? "✓" : "✗") << endl;
                exit(1);
            }
            
            // Auto-populate decks that weren't manually set up
            // Add all cards of that level that aren't on the board
            if (state.deck_level1.empty()) {
                cerr << "Auto-populating level1 deck with remaining cards..." << endl;
                for (const Card& card : all_cards) {
                    if (card.level == 1) {
                        // Check if this card is already on the board
                        bool on_board = false;
                ...
                cerr << "  Added " << state.deck_level1.size() << " cards to level1 deck" << endl;
            }
            
            if (state.deck_level2.empty()) {
                cerr << "Auto-populating level2 deck with remaining cards..." << endl;
                for (const Card& card : all_cards) {
                    if (card.level == 2) {
                        bool on_board = false;
                ...
                cerr << "  Added " << state.deck_level2.size() << " cards to level2 deck" << endl;
            }
            
            if (state.deck_level3.empty()) {
                cerr << "Auto-populating level3 deck with remaining cards..." << endl;
                for (const Card& card : all_cards) {
                    if (card.level == 3) {
                        bool on_board = false;
                ...
                cerr << "  Added " << state.deck_level3.size() << " cards to level3 deck" << endl;
            }
            
            cerr << "Setup complete, starting game" << endl;
            break;
        }
            ...
            vector<int> noble_ids;
            int id;
            while (iss >> id) {
                noble_ids.push_back(id);
            }
            
            cerr << "Setting up nobles: ";
            for (int noble_id : noble_ids) {
            ...
            cerr << "Setting up deck " << level << ": " << card_ids.size() << " cards" << endl;
            ...
            cerr << "Unknown setup command: " << command << endl;
        }
    }
}

// Process REVEAL command to manually place a card
bool processRevealCommand(GameState& state, const string& line, vector<Card>& all_cards) {
    istringstream iss(line);
    string command;
    int card_id;
    ...
    Card card = loadCardById(card_id, all_cards);
    if (card.id == 0) {
        cerr << "ERROR: Card " << card_id << " not found" << endl;
        return false;
    }
    
    cerr << "Revealing card " << card_id << " (level " << card.level << ")" << endl;
    ...
    if (card.level == 1) {
        ...
        cerr << "ERROR: Invalid card level: " << card.level << endl;
        return false;
    }
    ...
    if (state.pending_blind_reserve_player >= 0 && state.pending_blind_reserve_level == card.level) {
        // For blind reserves in replay mode, remove the card from deck and add to reserved
        ...
        for (auto it = deck->begin(); it != deck->end(); ++it) {
            if (it->id == card_id) {
                deck->erase(it);
                found_in_deck = true;
                cerr << "  (removed card " << card_id << " from level" << card.level << " deck)" << endl;
                break;
            }
        }
        
        if (!found_in_deck) {
            cerr << "ERROR: Card " << card_id << " not found in level" << card.level << " deck for blind reserve" << endl;
            return false;
        }
        
        // Replace the placeholder card in reserved with the actual card
        if (!state.players[player_idx].reserved.empty()) {
            state.players[player_idx].reserved.back() = card;
            cerr << "  Card " << card_id << " revealed for Player " << (player_idx + 1) << "'s blind reserve" << endl;
        }
        
        // Clear pending blind reserve and reveal expectation
    ...
    // For face-up card replacements, try to find and remove the card from the deck
    bool found_in_deck = false;
    for (auto it = deck->begin(); it != deck->end(); ++it) {
        if (it->id == card_id) {
            deck->erase(it);
    ...
    if (!found_in_deck) {
        cerr << "ERROR: Card " << card_id << " not found in level" << card.level << " deck (may be reserved, purchased, or already on board)" << endl;
        return false;
    }
    
    // Otherwise, place on board (for cards removed via BUY/RESERVE from board)
    // Determine insertion position based on last removed position
    int insert_pos = -1;
    if (card.level == 1 && state.last_removed_pos_level1 >= 0) {
        insert_pos = min(state.last_removed_pos_level1, (int)faceup->size());
        state.last_removed_pos_level1 = -1;  // Reset after use
    } else if (card.level == 2 && state.last_removed_pos_level2 >= 0) {
        insert_pos = min(state.last_removed_pos_level2, (int)faceup->size());
        state.last_removed_pos_level2 = -1;  // Reset after use
    } else if (card.level == 3 && state.last_removed_pos_level3 >= 0) {
        insert_pos = min(state.last_removed_pos_level3, (int)faceup->size());
        state.last_removed_pos_level3 = -1;  // Reset after use
    }
    
    // Insert at tracked position or append if no position tracked
    if (insert_pos >= 0) {
        faceup->insert(faceup->begin() + insert_pos, card);
        cerr << "  Card " << card_id << " placed at position " << insert_pos << " on board" << endl;
    } else {
        faceup->push_back(card);
        cerr << "  Card " << card_id << " placed at end of board (no position tracked)" << endl;
    }
    
    // Clear reveal expectation
    state.reveal_expected = false;
...
int main(int argc, char* argv[]) {
    ...
    // Check command-line arguments for replay mode
    // Usage: ./referee [seed]  OR  ./referee replay
    if (argc > 1 && string(argv[1]) == "replay") {
        ...
        // Output initial game states after setup
    ...
    while (!isGameOver(game)) {
        ...
        // Check if this is a REVEAL command (only valid in replay mode)
        if (move_string.find("REVEAL") == 0) {
            if (replay_mode) {
                processRevealCommand(game, move_string, all_cards);
                
                // Output updated game state after reveal
                cerr << "\n=== Game State for Player 1 (after reveal) ===" << endl;
                printJsonGameState(game, 1);
                
                cerr << "\n=== Game State for Player 2 (after reveal) ===" << endl;
                printJsonGameState(game, 2);
        ...
        // In replay mode, check if REVEAL is expected but missing
        if (replay_mode && game.reveal_expected) {
            cerr << "ERROR: Expected REVEAL command but received: " << move_string << endl;
            cerr << "Game terminated due to missing REVEAL command" << endl;
            cout << "ERROR: Missing REVEAL command in replay mode" << endl;
            return 1;
        }
        
        // Parse the move
        Move move = parseMove(move_string, current);
        
        // Validate the move
        ValidationResult result = validateMove(game, move);
        
        if (!result.valid) {
            cerr << "ERROR: Invalid move - " << result.error_message << endl;
            cerr << "Player " << (current + 1) << " loses by invalid move" << endl;
            
            // Output result: opponent wins
            cout << "WINNER: Player " << (2 - current) << endl;
            cout << "REASON: Player " << (current + 1) << " made invalid move" << endl;
            return 0;
        }
